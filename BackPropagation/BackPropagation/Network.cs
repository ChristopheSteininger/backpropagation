...
namespace BackPropagation
{
    class Network
    {
        ...
        {
            synOne = InitialiseSynArray(inputs, neurons);
            synTwo = InitialiseSynArray(neurons, outputs);
        ...
        private double[,] InitialiseSynArray(int a, int b)
        {
            ...
            for (int y = 0; y < b; y++)
            {
                for (int x = 0; x < a; x++)
                {
                    result[x, y] = random.NextDouble() * 0.1;
                }
            }
